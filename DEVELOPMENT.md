...
| File | Purpose |
| --- | --- |
| [`docker-compose.yml`](./docker-compose.yml) | Two services: `postgres` (PostgreSQL 16) + `api` (Node.js from `apps/api`). Healthchecks, persistent volume. |
...
# Restart the API (rebuilds after code changes if image uses a build cache)
docker compose restart api

# Rebuild the API image (after dep changes / Dockerfile edits)
...
### Prisma workflows (run inside the running `api` container)
...
| Changes to app code not reflected | The image is built once; rebuild with `docker compose up -d --build api`. |

## Related tickets

- [TIR-187](/TIR/issues/TIR-187) — this local Docker Compose environment.
- [TIR-179](/TIR/issues/TIR-179) — Render Blueprint (production counterpart, render.yaml).
